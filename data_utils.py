            ...
            if split == "train":
                ...
                if FLAGS.num_procs > 1:
                    ...
                    for shard, path in tqdm(enumerate(self.train)):
                        ...
                        num_shuffle = FLAGS.num_shuffle
                        datasets_shuffle = ["lm1b"]
                        if self.dataset not in datasets_shuffle:
                            num_shuffle = 1
                        ...
                        for shuffle in range(num_shuffle):
                            ...
                            basename = "train-{:03d}-{:02d}".format(shard, shuffle)
                            if self.dataset in datasets_shuffle:
                                np.random.shuffle(data_shard)
                                print("np.random.shuffling...")
...
def get_bin_sizes(
    ...
    tgt_len: int,
    cutoffs: List[int],
    std_mult: Tuple[float] = (2.5, 2.5, 2.5),
) -> List[int]:
    """
...
def get_lm_corpus(data_dir: str, dataset: str) -> Corpus:
    ...
    if exists(fn):
        ...
        if dataset in ["wt103", "wt2", "bookcorpus"]:
            # avoid brittle vocab (somehow wt103 did not need <unk>)
            # kwargs["special"] = ["<eos>"]
