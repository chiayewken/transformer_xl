...
flags.DEFINE_bool("track_mean", default=True, help="Trace mean loss during training.")
flags.DEFINE_string(
    "eval_ckpt_path",
    None,
...
flags.DEFINE_float("learning_rate", default=2.5e-4, help="Maximum learning rate.")
flags.DEFINE_float("clip", default=0.25, help="Gradient clipping value.")
# for cosine decay
flags.DEFINE_float("min_lr_ratio", default=0.01, help="Minimum ratio learning rate.")
flags.DEFINE_integer(
    "warmup_steps", default=0, help="Number of steps for linear lr warmup."
)

# Training parameters
flags.DEFINE_integer("train_batch_size", default=60, help="Size of train batch.")
...
flags.DEFINE_integer(
    "iterations", default=500, help="Number of iterations per repeat loop."
)
flags.DEFINE_integer(
    "save_steps", default=10000, help="number of steps for model checkpointing."
)
...
def metric_fn(loss):
    ...
    return {"perplexity": tf.metrics.mean(perplexity), "bpc": tf.metrics.mean(bpc)}


def get_model_fn(
    n_token: int,
    cutoffs: List[int],
    train_bin_sizes: List[int],
    eval_bin_sizes: List[int],
):
    def model_fn(features: dict, labels, mode, params: dict):
        ...
        loss, new_mems = model_with_lstm.transformer(
            dec_inp=inp,
            target=tgt,
            mems=mems,
            n_token=n_token,
        ...
        if mode == tf.estimator.ModeKeys.EVAL:
            if FLAGS.use_tpu:
                with tf.colocate_with(total_loss):
                    total_loss = (
                        tf.contrib.tpu.cross_replica_sum(total_loss)
                        / FLAGS.num_hosts
                        / FLAGS.num_core_per_host
                    )
            metric_loss = tf.tile(tf.reshape(total_loss, [1, 1]), [batch_size, 1])
            eval_spec = tf.contrib.tpu.TPUEstimatorSpec(
                mode=mode, loss=total_loss, eval_metrics=(metric_fn, [metric_loss])
            )
...
def get_cache_fn(mem_len: int):
    def cache_fn(batch_size: int) -> List:
        mems = []
