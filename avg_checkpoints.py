...
flags.DEFINE_string(
    "prefix", "", "Prefix (e.g., directory) to append to each checkpoint."
)
flags.DEFINE_string(
    "output_path", "/tmp/averaged.ckpt", "Path to output the averaged checkpoint to."
)
...
def main(_):
    ...
    if FLAGS.checkpoints:
        ...
        assert FLAGS.prefix, (
            "Prefix must be provided when averaging last" " N checkpoints"
        )
    ...
    if not checkpoints:
        if FLAGS.checkpoints:
            ...
            raise ValueError(
                "Could not find checkpoints at %s" % os.path.dirname(FLAGS.prefix)
            )
